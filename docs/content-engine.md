...
## Directory structure
    ...
    content/
      data/
        posts.json          (legacy - still used as A2 fallback)
        projects.json
        site.json
        updates.json
      posts/
...
mount with `replaceChildren`. None use `innerHTML` on a live element.
    ...
    js/writing.js      writing.html      getSortedCollection("posts","desc")
...
1. Fetch from the engine (via the Router) and from legacy JSON
   (`content/data/posts.json` or `content/data/projects.json`) in parallel,
   using `Promise.all`.
2. Normalize both result sets to a common shape.
3. Choose whichever collection is **larger**.
4. Render the winner.

Why it exists: during the migration from legacy JSON to Markdown-in-git, the
engine held fewer items than the legacy file. Rendering the engine's smaller
set would have visibly shrunk the homepage (5 posts down to 1) or the archive
(30 posts down to 1) mid-migration. The A2 pattern keeps the page visually
identical while migration is incomplete.

Why "larger" and not "non-zero": an earlier draft of the pattern said "fall
back only if the engine returns zero items." That was wrong -- with 1 engine
item and 30 legacy items, the archive would still have shown only 1 post. The
"larger wins" rule ensures the migration is invisible.

Why it self-retires: once the engine holds at least as many items as the
legacy file, the comparison picks the engine on every load. The legacy fetch
still runs (it is part of `Promise.all`) but its result is discarded. When the
legacy files are eventually deleted, the `catch` branch handles the 404 and
the engine path continues unchanged.

After Milestone 15, `posts` has flipped: the engine now holds 32 items against
legacy's 31, so the engine wins on every page. `projects` has not flipped yet
(engine: 1, legacy: 4).
...
- preserves the year-grouping heading hierarchy on `writing.html`,
...
`js/content/loader.js`, `js/content/render.js`, and `js/content/posts-page.js`
are an older attempt at the same idea. They predate the current engine, are
not referenced by any HTML file, and can be deleted. `content/data/*.json` is
still load-bearing (the A2 fallback reads it), so it stays until migration is
declared complete.
...
- **Manifest-only fetch mode.** Every archive page (`writing.html`) and the
  post page currently load every post's full markdown body just to render
  titles and dates. That is 32 fetches on `writing.html`. A future loader mode
...
- **Deletion of `js/content/`.** Confirmed dead code (see above), but not yet
  removed.
...
For readers picking this up cold, the current state was reached through:
    ...
    M11   writing.html wired to engine (A2 fallback)
